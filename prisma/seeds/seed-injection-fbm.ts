 ...
 */
function extractMeaningfulEvents(events: any[]): any[] {
  const meaningfulEvents: any[] = [];
  let stepNumber = 0;

  for (const event of events) {
    // Skip session events and non-interactive events
    if (event.type === 'sessionStart' || event.type === 'sessionEnd') continue;
    if (event.type === 'scroll' && !event.element) continue;
...
function detectValueType(value: string): string {
  ...
  // Year pattern
  if (/^(19|20)\d{2}$/.test(value)) return 'vehicle_year';
  
  // Price pattern
  if (/^\$?[\d,]+\.?\d*$/.test(value)) return 'vehicle_price';
  
  // VIN pattern (17 chars)
  if (/^[A-HJ-NPR-Z0-9]{17}$/i.test(value)) return 'vehicle_vin';
  
  // Mileage pattern
  if (/^[\d,]+\s*(miles?|mi)?$/i.test(value)) return 'vehicle_mileage';
