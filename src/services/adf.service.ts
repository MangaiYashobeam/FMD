...
export class ADFService {
  ...
  generateADFXML(data: ADFLeadData): string {
    ...
    xml.push(`    <id sequence="1" source="${this.escapeXml(data.source || 'FaceMyDealer')}">${this.escapeXml(data.id)}</id>`);
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>This lead was generated by FaceMyDealer | ADF XML attached for DMS import</p>
    </div>
  </div>
</body>
</html>
  ...
  private mapLeadSourceToADF(source: string): string {
    ...
    return sourceMap[source] || 'FaceMyDealer';
  }
